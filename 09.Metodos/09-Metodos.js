...
function masImpuestos(array){
    ...
    return array.map(objeto => {
        const impuestos = Math.round(objeto.precio * 0.10); // Redondear a número entero
        return {...objeto, impuestos};
    });
}
...
function examenIngreso(array){
    ...
    // Tu codigo:
    return array.filter(alumno => alumno.nota >= 70 && alumno.inscripcion);
}
